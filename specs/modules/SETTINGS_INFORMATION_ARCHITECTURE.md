# FooCrate 设置项与信息架构规格草案

- 状态：已验收
- 版本：0.8
- 最后更新：2026-07-15
- 对应任务：`17 重构 FooCrate 设置菜单与配置界面`
...
| Foobox 设置 | Foobox 行为 | FooCrate 建议 |
| --- | --- | --- |
| Smooth scrolling | 对脚本列表做缓动滚动 | 不增加；FooCrate 保持原生滚轮行为，避免额外动画状态 |
...
| Selection 子菜单 | 把选择操作再包一层右键菜单 | 不增加；FooCrate 保持标准 foobar2000 上下文菜单 |
| Play next playlist automatically | Default playback order 下跨列表续播 | 不建议；跨列表副作用较大，Playback Order 已是正式控制入口 |
| Queue Content playlist | 队列非空时建立镜像播放列表 | 不增加；FooCrate 已有独立 Playback Queue Workspace |
...
FooCrate 当前已有列显隐、顺序、复制、删除、Label、Display format、Sort format、Alignment 和 Width weight。任务 17 建议：
...
FooCrate 已有 Group key、Sort、Title left/right、Subtitle left/right 和 Artwork。任务 17 建议：
...
| Foobox 设置 | FooCrate 处理 |
| --- | --- |
| Show pseudo main menu bar | 不增加；FooCrate 使用 Columns UI 正式 Main Menu，任务 14 已冻结结构 |
| Scrollbar width: System / Narrow | 不增加；任务 15 已验收 FooCrate 细滚动条语义，任务 18 再做系统可访问性回退 |
...
| Except background / Except ESLyric highlight | 不增加；FooCrate 主题使用统一语义色，不拆成互相矛盾的局部来源 |
| Cover colour threshold / background chroma | 不暴露算法参数；取色必须保持可读性和确定性 |
| Colorful seek bar | 不增加；进度条使用当前主题 Accent |
| Cover filenames for directory groups | 不增加；FooCrate 通过 foobar2000 Artwork API 读取，不自行扫描文件名 |
| Cover cache dimension | 不增加；缓存按实现安全上限自动管理，不让用户用像素值调资源占用 |
| Write rating to file tag | 不增加；Playback Statistics 是 FooCrate 唯一评分数据源，避免静默改写文件标签 |
| Right panels follow cursor | 建议新增为 `Right panel follows: Playing track / Playlist selection`；封面主题仍跟随正在播放项，避免浏览选择时整套颜色不停变化 |
| More track info on titlebar | 建议改造成 `Now Playing summary format` 高级设置；作用于 FooCrate 顶部摘要，不改 Windows 标题栏 |
| Enable D2D rendering | 不增加；FooCrate 的渲染后端是实现要求，不是用户功能开关 |

### 2.5 Playlist layouts

Foobox 为播放列表保存独立布局，布局包含：分组模式、是否显示组封面、自动折叠、折叠/展开组头高度、默认折叠状态和列宽/显隐；并允许删除当前布局和清理无效布局。

这是最值得 FooCrate 借鉴、但也是任务 17 中范围最大的一项。建议采用更清楚的模型：
...
FooCrate 的 Album-only 产品边界不变，不增加 Artist、Folder、Genre、Tag 或通用分类模式。建议只新增：
...
Foobox 暴露新列表排序格式、保留列表锁定、网格线、删除确认、触摸控制和 Filter。FooCrate 建议：
...
Foobox 搜索框有 Source、Auto-validation、Scope、Keep Playlist、Follow Cursor、History 和 Reset Button。FooCrate 已明确把搜索交给 foobar2000 `Ctrl+F`，因此这些设置全部不进入 FooCrate。
...
- Colour mode：Follow Windows / FooCrate preset / Follow album artwork。
- Preset：四套主题，仅在 FooCrate preset 下启用。
...
| 页面 | 内容 |
| --- | --- |
| `FooCrate` | Playback controls、基础界面选项、Appearance |
...
用户要求附加行内容与 Foobox 默认行为一致，但不得复制 Foobox 的 JavaScript 实现。FooCrate 使用自身 C++ 标题格式模型复现以下默认映射：

| FooCrate 列 | Secondary display format |
| --- | --- |
| `#` | `$if2(%play_count%,0)` |
| `Title` | `$if2(%album artist%,'Unknown artist')` |
| `Album` | `$if2(%genre%,'Other')` |
| `Length` | Foobox Time 附加行的动态码率表达式；FooCrate 以等价安全标题格式实现 |
| 其他内置列 | 空；用户可自行填写 |
...
| 页面 | 分区 |
| --- | --- |
| `FooCrate` | Playback controls、Interface、Appearance |
...
25. `Album Artist / Album / Disc` 的默认 Sort 为 Album Artist → Date/Year → Album → Disc → Track → Title；FooCrate 新建 Auto Playlist 使用同一默认 Sort。已有 Auto Playlist 的来源规则不静默迁移，用户通过该列表 Properties 修改一次 Sort，避免组件擅自改写查询配置。
