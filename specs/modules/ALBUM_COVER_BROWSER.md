...
- 每项显示正方形 Front Cover、第一行 Album、第二行 Album Artist；标题超长单行省略，完整 Album/Artist/Year/曲目数放入提示文本。
- 缺封面使用稳定占位，加载中只在对应单元显示，不改变网格尺寸；选择以灰底和边框共同表达，不能只依靠颜色。
...
- 这个桥接列表会出现在 foobar2000/Columns UI 的播放列表标签和未来 Playlist Browser 中；Refrain 将它标为保留工作列表并禁止从自己的管理界面删除或编辑。不会按专辑创建多个列表。
