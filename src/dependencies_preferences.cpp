...
private:
    ...
    void layout() const { RECT r{}; GetClientRect(m_window,&r); const auto width=static_cast<int>(r.right); for(std::size_t i=0;i<m_labels.size();++i){MoveWindow(m_labels[i],16,20+static_cast<int>(i)*42,std::max(1,width-190),24,TRUE);MoveWindow(m_buttons[i],std::max(16,width-166),14+static_cast<int>(i)*42,150,28,TRUE);}MoveWindow(m_note,16,154,std::max(1,width-32),40,TRUE);}
    preferences_page_callback::ptr m_callback; std::vector<DependencyStatus> m_dependencies; std::vector<HWND> m_labels,m_buttons; HWND m_window{},m_note{}; WNDPROC m_original{}; HFONT m_font{};
};
