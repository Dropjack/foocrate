...
int main() {
    ...
    const auto firstRun = migrateSettings({0, 0, true, true, true, 0, false, 500});
    expect(firstRun.versionToKeep == 2, "version zero must migrate to two");
    expect(firstRun.rewriteKnownValues, "version migration must be persisted");

    const auto invalid = migrateSettings({2, 77, false, false, false, 77, true, 999});
    ...
    const auto remaining = migrateSettings({2, 1, true, false, false, 1, true, 640});
    ...
    const auto future = migrateSettings({9, 77, false, true, true, 77, false, 999});
