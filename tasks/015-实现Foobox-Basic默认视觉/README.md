...
1. FooCrate 继续从窗口客户区和当前 DPI 得到逻辑宽高。
...
10. Track Details 在当前下区视口内重新钳制滚动位置并硬裁剪；ESLyric 伪透明背景由 FooCrate 父窗口提供与 Now Playing 一致的背景色。
11. Album Cover Browser 使用唯一的无间距正方形封面墙；默认无文字，Hover/Selected 才在封面内部底部显示单行 Album 标题，Selected 保持撞色描边。
12. FooCrate 不再提供 Playlist Filter：删除输入控件、清除按钮、筛选状态、可见行筛选模型、专用拖放限制和布局占位；`Playlists` 标题下直接显示完整列表。foobar2000 `Ctrl+F` 与原生 Album List 入口不变。
13. Lyrics View 不能只依赖父窗口伪透明；ESLyric 播放时主动绘制的背景通过其可选 Automation 接口同步为 FooCrate 当前背景色。接口不可用时保持原 ESLyric 行为，不影响歌词功能。
...
- 输入：用户最新决定“FooCrate 不要 Filter；不是隐藏”。
...
- 产物：有译文时默认保留原文与译文的产品规则；当前 NetEase 脚本与 KRC 解析器已执行这一行为，因此 FooCrate 无需新增虚假的双行开关。
...
- 2026-07-14：组件包 `dist/FooCrate-0.1.0.fb2k-component` 已生成，SHA-256 为 `D6120FC970C22C78A6E124AF7F53AB474B558E04F83A5582E429C5E183784385`。Release DLL 只部署到 `.local/foobar-dev`，构建与部署 SHA-256 同为 `68EAF45C81B96AEE4CD736C203BA083CD0024B1B9BD3C00334592D982FB7FAD5`，运行进程确认加载该 DLL。
...
- 2026-07-14：Track Details 在每次绘制前按当前视口重新钳制滚动位置，并对整个下区使用 Direct2D 硬裁剪。FooCrate 同时处理父窗口 `WM_ERASEBKGND`/`WM_PRINTCLIENT`，从单一背景色值为 ESLyric 伪透明模式提供与 Now Playing 相同的背景。
...
- 2026-07-14：用户首次检查确认白色多行信息框来自 FooCrate 旧 Album Grid tooltip，并要求删除；封面墙此后只使用内部单行 Hover/Selected 标题条，其他按钮 tooltip 不受影响。
...
- 2026-07-14：用户随后作出覆盖此前方案的最终决定：FooCrate 不提供 Playlist Filter，且不是隐藏。实现完整删除控件、状态、筛选模型、布局占位、专用交互分支及测试；此前 Filter 视觉记录只作为决策历史保留，SVG 清除图标计划一并取消。
...
- 2026-07-14：FooCrate 新增可选背景同步适配：从已托管 ESLyric 子窗口取得实际加载模块，经导出 `DllGetClassObject` 创建内嵌 Automation 控制器，并以 ARGB `#FFF4F1EA` 同步当前背景。失败只记录兼容提示，不改变托管、歌词或 Track Details；不读取安装路径、不写 ESLyric 配置，也不覆盖背景类型、字体或歌词颜色。
...
Now Playing 封面采用宽高共同约束的正方形布局，不再保留固定左右留白；Track Details 以硬裁剪保证不越过下区。ESLyric 的背景类型属于外部组件私有设置，FooCrate 不改写其私有配置；集成使用 ESLyric 自带伪透明模式，并由 FooCrate 提供和 Now Playing 相同的父背景。
