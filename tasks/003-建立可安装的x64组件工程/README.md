...
建立可以从干净构建目录重复生成 Windows x64 原生 foobar2000 组件的正式工程入口。完成后，Codex 能构建 Debug/Release、运行基础测试、生成标准 `.fb2k-component`、安全部署到隔离实例，并证明包中只有 FooCrate 自己的 DLL。

本任务只建立组件身份和工程能力，不注册任何空白 Columns UI 面板。用户在 Components 页面会看到 FooCrate 组件信息，但真正的根面板与播放控制从步骤 04 开始实现。

## 明确不做

- 不实现主界面、播放控制、设置页面或任何占位 UI。
- 不部署到 C 盘日常安装或 `D:\Dev\foobar2000`。
- 不把 SDK、foobar2000、Columns UI、ESLyric 或 Playback Statistics 二进制打进 FooCrate 包。
...
2. 工程把第三方 SDK 源码编译到已忽略的构建目录；FooCrate 代码只注册组件名称、版本、说明和固定文件名，不创建 UI 服务。
3. 稳定组件 GUID 与版本常量保存在源代码中，基础测试检查它们没有被意外清空或改变格式。
4. Debug 和 Release 使用相同源代码、动态 MSVC 运行库和 x64 ABI；FooCrate 自有代码启用严格警告，第三方 SDK 警告不冒充 FooCrate 新警告。
5. 打包脚本只接受当前 Release 构建中的 `foo_crate.dll`，在根目录 `dist/` 生成容易找到的 `FooCrate-0.1.0.fb2k-component`；包内 DLL 必须直接位于根目录，随后审计条目、模拟安装布局和哈希。
...
- 用户检查方法：无需阅读 C++；确认本步骤只会在 Components 页面增加 FooCrate。
...
- 用户检查方法：无需手工编译。
- 通过标准：Debug/Release 成功；测试通过；FooCrate 自有代码无新增警告。
- 状态：已完成
...
- 通过标准：包中只有根目录 `foo_crate.dll`，模拟解压后 DLL 直接位于 `user-components-x64/foo_crate/`；非法目标全部拒绝；仅 dev 收到组件。
- 状态：已完成

### 步骤 4：开发实例人工验收

- 输入：已部署 FooCrate 的 `foobar-dev`。
- 动作：用户查看组件信息，执行卸载/重装并确认无空白 FooCrate 面板。
- 产物：人工验收记录。
...
- [x] Components 页面显示 FooCrate 名称、版本和说明。
- [x] FooCrate 可以卸载并由生成包重新安装。
- [x] 安装后 foobar2000 正常，Columns UI 布局没有被覆盖。
- [x] 本步骤没有出现空白或无功能的 FooCrate UI 面板。
...
- 2026-07-13：Debug 与 Release 全量构建成功；FooCrate 自有代码使用 `/W4 /WX` 且无警告。
- 2026-07-13：Debug/Release 的 `component_identity` CTest 均通过；全新 `build/clean-check` 目录也完成 Release 构建、测试和打包。
- 2026-07-13：第三方未修改源码产生 5 处 C4996：PFC 的 `GetVersion`、shared 的 `GetVersionExW`、SDK 的 3 处旧 metadb 调用。它们来自官方 SDK 自身，未通过修改或全局屏蔽第三方代码伪装修复，不影响 FooCrate `/WX` 门槛。
- 2026-07-13：Win32 配置被 CMake 拒绝；打包脚本拒绝 Debug DLL；部署脚本拒绝未知实例和 `build/` 外 DLL。
- 2026-07-13：首次安装包误用 `x64/foo_crate.dll`。用户通过 Components 页面安装重启后组件消失；磁盘证据显示 foobar2000 将其原样解压成 `profile/user-components-x64/foo_crate/x64/foo_crate.dll`，比加载位置多一层目录。任务因此从待验收退回实现中，打包约束改为根目录 DLL，并增加模拟安装布局测试。
- 2026-07-13：修正版安装包审计只有根目录 `foo_crate.dll`，模拟解压后 DLL 直接位于组件目录根部；反向测试确认旧式 `x64/foo_crate.dll` 包会被验证脚本拒绝。
...
- 2026-07-13：Release DLL 只部署到 `foobar-dev`，部署后哈希一致；后台启动保持运行 5 秒，随后通过正式 Exit 命令正常退出，未生成新崩溃文件；`foobar-test` 未部署 FooCrate。
...
- `src/foo_crate.rc`：Windows DLL 版本资源。
...
- 初始工程版本使用 `0.1.0`；它代表工程骨架，不是 FooCrate 1.0 产品完成度。
- x64 单组件包中的 `foo_crate.dll` 直接位于包根目录；目标架构由安装实例的 `user-components-x64` 存储位置决定，包内不重复增加 `x64/` 层。
- SDK 源文件组合、系统链接库与包格式已经由两个构建配置和全新目录重建证明。

## 用户人工验收

1. 启动 `.local/foobar-dev/foobar2000.exe`。
2. 打开 `File > Preferences > Components`，确认列表存在 `FooCrate 0.1.0`。
3. 双击或查看组件信息，确认说明以 `Native integrated Columns UI component for foobar2000.` 开头。
4. 确认 Columns UI 当前布局没有变化，布局编辑器中也没有可添加的 FooCrate 空白面板。
5. 在 Components 页面卸载 FooCrate并正常重启。
6. 在 `foobar-dev` 的 Components 页面点击 `Install...`，选择根目录 `dist/FooCrate-0.1.0.fb2k-component`，确认并重启，再确认 `FooCrate 0.1.0` 恢复。不要从资源管理器双击安装包，以免系统文件关联把它交给 C 盘日常实例。
