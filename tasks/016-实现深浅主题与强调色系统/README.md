...
提供两套浅色、两套深色的完整预设，并把颜色选择收敛成 `Follow Windows`、`FooCrate preset`、`Follow album artwork` 三种互斥模式。封面模式生成完整语义主题；FooCrate 不读取或改写 Columns UI 配色。
...
3. `FooCrate preset` 使用四套固定主题之一；只有此模式允许选择 Preset。
...
6. Preferences 草稿即时预览；`Apply` 持久化，`Cancel` 恢复，`Reset page` 回到 `FooCrate preset + 漫雾青`。
7. 主题变化只重建颜色画刷并同步 ESLyric 背景，不重置播放、列表、滚动、分隔、Workspace 或封面缓存。
8. 设置版本 5 确定性迁移版本 4：旧 Windows 保留为 Follow Windows，旧 FooCrate 和 Columns UI 都迁移为 FooCrate preset；旧 Accent source 和 Custom colour 停用。

## 用户人工验收

1. 打开 `Preferences > Display > FooCrate`，确认 Appearance 只有 `Colour mode` 和 `Preset` 两行。
2. 选择 `FooCrate preset`，确认 Preset 可用，四套名称完整；逐一切换并检查浅色两套、深色两套。
3. 选择 `Follow Windows`，确认 Preset 禁用；切换 Windows 应用浅/深色后确认 FooCrate 跟随。
...
- 2026-07-15：Preferences 已收敛为两行；Preset 只在 FooCrate preset 模式启用。高对比度仍拥有最高优先级。
- 2026-07-15：设置架构升级到版本 5，复用旧基础来源 GUID 保存 Colour mode，并覆盖旧 FooCrate、Windows、Columns UI 的迁移测试。
